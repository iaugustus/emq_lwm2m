...
handle_call(info, From, State = #state{proto = ProtoState, peer = Channel}) ->
    ProtoInfo  = emq_protocol:info(ProtoState),
...
handle_call(stats, _From, State = #state{proto = ProtoState}) ->
    {reply, lists:append([emqttd_misc:proc_stats(), emq_protocol:stats(ProtoState)]), State};
...
handle_call(session, _From, State = #state{proto = ProtoState}) ->
    {reply, emq_protocol:session(ProtoState), State};
...
proto_init(ClientId, Username, Password, Channel, KeepAliveInterval) ->
    ...
    Proto = emq_protocol:init(Channel, SendFun, PktOpts),
    ConnPkt = #mqtt_packet_connect{client_id  = ClientId,
                                   username   = Username,
                                   password   = Password,
                                   clean_sess = true,
                                   keep_alive = KeepAliveInterval},
    case emq_protocol:received(?CONNECT_PACKET(ConnPkt), Proto) of
        {ok, Proto1}                              -> {ok, Proto1};
...
proto_subscribe(Topic, Proto) ->
    ...
    case emq_protocol:received(?SUBSCRIBE_PACKET(1, [{Topic, ?QOS1}]), Proto) of
        {ok, Proto1}  -> Proto1;
        Other         -> error(Other)
    end.
...
proto_unsubscribe(Topic, Proto) ->
    ...
    case emq_protocol:received(?UNSUBSCRIBE_PACKET(1, [Topic]), Proto) of
        {ok, Proto1}  -> Proto1;
        Other         -> error(Other)
    end.
...
proto_publish(Topic, Payload, Proto) ->
    ...
    Publish = #mqtt_packet{header   = #mqtt_packet_header{type = ?PUBLISH, qos = ?QOS0},
                           ...
                           payload  = Payload},
    case emq_protocol:received(Publish, Proto) of
        {ok, Proto1}  -> Proto1;
        Other         -> error(Other)
    end.
...
proto_deliver_ack(#mqtt_message{qos = ?QOS1, pktid = PacketId}, Proto) ->
    case emq_protocol:received(?PUBACK_PACKET(?PUBACK, PacketId), Proto) of
        {ok, NewProto} -> NewProto;
        Other          -> error(Other)
    end;
proto_deliver_ack(#mqtt_message{qos = ?QOS2, pktid = PacketId}, Proto) ->
    case emq_protocol:received(?PUBACK_PACKET(?PUBREC, PacketId), Proto) of
        {ok, NewProto} ->
            case emq_protocol:received(?PUBACK_PACKET(?PUBCOMP, PacketId), NewProto) of
